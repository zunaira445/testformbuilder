...
<head>
    ...
    <style>
        ...
        .plan-card.popular { border-color: #1e40af; transform: scale(1.03); box-shadow: 0 12px 36px rgba(30,64,175,.15); }
        ...
        <div class="collapse navbar-collapse" id="navMenu">
            <ul class="navbar-nav ms-auto align-items-lg-center gap-1 mt-3 mt-lg-0">
                ...
                <li class="nav-item mt-2 mt-lg-0">
                    <a href="{{ route('student.login') }}" class="btn btn-outline-primary btn-sm px-3 w-100 w-lg-auto">Student Login</a>
                </li>
                <li class="nav-item mt-2 mt-lg-0">
                    <a href="{{ route('instructor.login') }}" class="btn btn-primary btn-sm px-3 w-100 w-lg-auto">Instructor Login</a>
                </li>
            </ul>
        </div>
    </div>
    ...
    <div class="container">
        <div class="row align-items-center g-4">
            <div class="col-lg-6 text-center text-lg-start">
                <span class="badge bg-warning text-dark mb-3 px-3 py-2 fs-7">
                    🎓 Student Welfare Foundation
                </span>
                <h1 class="hero-title mb-3">Online Test Platform<br>Made for Pakistan</h1>
                <p class="hero-sub mb-4">
                    Create professional exams, conduct tests securely with anti-cheat, and analyze results — all in one powerful platform.
                </p>
...
<section id="features" class="py-5 bg-light">
    <div class="container py-3">
        ...
        <div class="row g-4">
            @foreach([
                ['icon'=>'bi-shield-lock-fill','color'=>'bg-danger bg-opacity-10 text-danger','title'=>'Anti-Cheat System','desc'=>'Detects tab switching and copy attempts. Auto-submits after 3 violations.'],
                ['icon'=>'bi-clock-fill','color'=>'bg-warning bg-opacity-10 text-warning','title'=>'Smart Timer','desc'=>'Countdown timer with automatic submission when time expires.'],
                ['icon'=>'bi-bar-chart-fill','color'=>'bg-success bg-opacity-10 text-success','title'=>'Instant Analytics','desc'=>'Scores, ranks, percentages — everything auto-calculated on submission.'],
                ['icon'=>'bi-file-earmark-arrow-down-fill','color'=>'bg-info bg-opacity-10 text-info','title'=>'Export Results','desc'=>'Download results as PDF, Excel, or CSV with one click.'],
                ...
                ['icon'=>'bi-collection-fill','color'=>'bg-secondary bg-opacity-10 text-secondary','title'=>'Question Bank','desc'=>'Save MCQs for reuse across multiple tests and exams.'],
            ] as $f)
...
<section id="how" class="py-5">
    <div class="container py-3">
        ...
        <div class="row g-4 justify-content-center">
            @foreach([
                ['step'=>'1','title'=>'Register','desc'=>'Create a free account as instructor or student in under 2 minutes.','icon'=>'bi-person-plus'],
                ['step'=>'2','title'=>'Create or Join Test','desc'=>'Instructors build tests; students enter a test code to join.','icon'=>'bi-journal-plus'],
                ['step'=>'3','title'=>'Take the Exam','desc'=>'Real-time timer, anti-cheat monitoring, and auto-save answers.','icon'=>'bi-pencil-square'],
                ['step'=>'4','title'=>'View Results','desc'=>'Instant scores, rankings, and detailed analysis after submission.','icon'=>'bi-trophy'],
            ] as $step)
...
<section id="pricing" class="py-5 bg-light">
    <div class="container py-3">
        ...
        <div class="row g-4 justify-content-center align-items-stretch">
            @foreach([
                ['name'=>'Basic','price'=>'2,000','color'=>'secondary','icon'=>'bi-box','features'=>['10 Tests / month','100 Students','MCQ Builder','Basic Timer'],'popular'=>false],
                ['name'=>'Pro','price'=>'3,500','color'=>'primary','icon'=>'bi-rocket-takeoff','features'=>['50 Tests / month','500 Students','Anti-Cheat System','Question Bank','Excel/PDF Export'],'popular'=>true],
                ['name'=>'Max','price'=>'7,000','color'=>'purple','icon'=>'bi-trophy','features'=>['Unlimited Tests','Unlimited Students','All Pro Features','Full Analytics','Priority Support'],'popular'=>false],
            ] as $plan)
            <div class="col-md-4">
                <div class="plan-card {{ $plan['popular'] ? 'popular' : '' }} h-100 bg-white shadow-sm p-4">
                    @if($plan['popular'])
                    ...
                    <div class="text-center mb-3">
                        <div class="icon-box bg-{{ $plan['color'] === 'purple' ? 'primary' : $plan['color'] }} bg-opacity-10 text-{{ $plan['color'] === 'purple' ? 'primary' : $plan['color'] }} mx-auto mb-3" style="{{ $plan['color']==='purple'?'background:rgba(124,58,237,.1)!important;color:#7c3aed!important':'' }}">
                            <i class="bi {{ $plan['icon'] }}"></i>
                        </div>
                        <h4 class="fw-bold mb-0" style="{{ $plan['color']==='purple'?'color:#7c3aed':'' }}">{{ $plan['name'] }}</h4>
                        <div class="mt-2">
                            <span class="fs-2 fw-bold">PKR {{ $plan['price'] }}</span>
                    ...
                    <a href="{{ route('pricing') }}" class="btn btn-{{ $plan['popular'] ? 'primary' : 'outline-secondary' }} w-100 fw-semibold">
                        Get {{ $plan['name'] }}
                    </a>
                </div>
            </div>
        ...
        <p class="text-center text-muted small mt-4">
            <i class="bi bi-info-circle me-1"></i>Yearly plans available with up to 52% discount. Payment via JazzCash, EasyPaisa, or Binance Pay.
        </p>
    </div>
</section>
        ...
        <div class="row align-items-center g-3">
            <div class="col-md-4 text-center text-md-start">
                <div class="d-flex align-items-center gap-2 justify-content-center justify-content-md-start mb-1">
                    <div style="width:32px;height:32px;border-radius:7px;background:linear-gradient(135deg,#1e40af,#0ea5e9);display:flex;align-items:center;justify-content:center;font-size:.7rem;font-weight:900;color:#fff">SWF</div>
                    <span class="fw-bold">SWF PORTAL</span>
                </div>
                <p class="text-white-50 small mb-0">Student Welfare Foundation</p>
            </div>
            <div class="col-md-4 text-center">
                <p class="text-white-50 small mb-0">&copy; {{ date('Y') }} Student Welfare Foundation.<br>All rights reserved.</p>
            </div>
            <div class="col-md-4 text-center text-md-end">
                <a href="https://wa.me/923148379859" target="_blank" class="text-white-50 text-decoration-none d-block d-md-inline me-md-3 mb-1 mb-md-0 small">
